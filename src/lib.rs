...
impl ChxClient {
    ...
    pub async fn connect(addr: &str) -> Result<Self, ChxError> {
        let stream = TcpStream::connect(addr)
            .await
            .map_err(|e| ChxError::ConnectionError(e.to_string()))?;
    ...
    pub async fn parse_response(&mut self) -> Result<String, ChxError> {
        ...
        if let Some(stripped) = response.strip_prefix("!e") {
            ...
            return Ok("".to_string()); // Command acknowledged, no data
        }
    ...
    pub async fn get(&mut self, key: &str) -> Result<Option<String>, ChxError> {
        ...
        match self.parse_response().await {
            ...
            Err(ChxError::ResponseError(msg)) if msg == "Key not found" => Ok(None),
            Err(e) => Err(e),
        }
    }
    ...
    pub async fn set(&mut self, key: &str, value: &str) -> Result<(), ChxError> {
        ...
        self.send_command(&command).await?;
        self.parse_response().await?; // Expecting acknowledgment
        Ok(())
    }
...
/// }
/// ```
pub async fn server(host: &String, port: &u16, expire_time: u64) -> Result<(), ChxError> {
    ...
    eprintln!("Server successfully bound to {}:{}", host, port);
    ...
    tokio::spawn(async move {
        loop {
            ...
            eprintln!("Client connected from {:?}", socket.peer_addr());
